...
- **Resolve a conflict by reading the region and choosing, never by pattern-matching across the markers.** The two sides of a conflict are near-identical by construction — that is why they conflicted — so a regex or scripted substitution written to strip the markers has no way to tell which closing brace, tag or clause belongs to which side, and it silently takes the wrong one. The result compiles-looking and wrong: a scope closed one line early, a block from the wrong side, a hunk swallowed whole. Before `git rebase --continue`, confirm the file still parses and the delimiters balance. The cost of skipping that is paid later and elsewhere — a file that no longer loads fails in whatever runs it next, usually a suite or a build that reports the symptom nowhere near the rebase.
...
- **A three-dot diff is the sharp edge of that, and it fails as an accusation.** `git diff <base>...HEAD` resolves its merge-base from the *remote-tracking* ref, so while `origin/<base>` is stale the diff silently absorbs every commit that has landed on the base since your last fetch and presents them as work on this branch. The output is not merely inflated: it names other people's unrelated commits as if someone had pushed them onto your branch, which is a claim about a colleague made from a ref you forgot to update. Fetch the base immediately before reading one. When the question is "what is actually in this change", prefer the forge's own API, which computes against the real base tip and cannot be locally stale, and reconcile the two before saying anything about who put a commit where. The `[pr-state]` hook reports `fetch-age`; a warning on that line means every ahead/behind count and three-dot reading in that checkout is suspect until you fetch.
- **Pick the diff form from the question, not from habit — and fetch either way.** "What does this branch add?" is three-dot (`<base>...<branch>`), which reads from the merge base. Two-dot (`<base> <branch>`) compares the two tips, so everything that landed on the base since the branch forked shows up *reversed*, as though the branch had deleted or rewritten it. On a branch that has fallen behind, that turns a handful of real changes into a list dominated by other people's work, and the reader cannot tell which is which. The tell is unrelated subsystems appearing in a focused change; if the list surprises you, check the form before explaining the surprise.
...
- **When dating a release by "which one contains this commit", enumerate every candidate, and cross-check the answer against the earliest observation of the effect.** Testing a handful of plausible release refs and taking the first hit gives a date that is *too late* whenever the carrying release was not in the handful — and the error is invisible, because a later release genuinely does contain the commit. The tell is free and usually already in front of you: if the effect was first observed before your claimed arrival, the claim is wrong, not the observation. Ask "what is the complete set of releases that could have carried this" (`git branch -r --contains`, or ancestry over every merge into the deploying branch), and if a first sighting predates the date you land on, go back rather than explaining the gap away. This matters because the date usually goes into a durable artifact — a ticket, a contaminated-window note, a before/after measurement — where nobody re-derives it.
