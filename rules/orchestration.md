...
**A process you did not start is not yours to restart — find out whether you can before you stop it.** Killing a service to clear a suspected bad state assumes the restart is free, and it often is not: the running one may have been started under an environment, a flag or a working directory that no longer exists, and nothing about it while it is up will tell you. The moment it refuses to come back you have taken away something that was working, in the middle of whatever the user was doing. Establish the restart path first — the exact command and everything it needs — and prefer the narrower fix if one exists, since a stale cache or artifact can usually be invalidated without stopping anything. Say you are about to stop it, and if it will not restart, say that immediately rather than after several attempts.

**Stop a named process, never a pattern.** `pkill -f` matched against a framework or runtime name catches every sibling using the same tooling — other projects, other ports, work nobody asked you to touch — and it reports the same success either way. Resolve the port to a pid first and signal that, then confirm what is still listening.

Before reporting a service down, check the **port**, not the process name: `lsof -nP -iTCP:<port> -sTCP:LISTEN`, or curl its status endpoint. A `pgrep` for a plausible-looking name is a guess — Metro, Vite and friends run under `node` with argv that rarely contains the word you searched for, so a name miss reads as "not running" and sends the diagnosis down the wrong path entirely. And a listening port is not evidence it is *your* app: a dev server on the conventional port may belong to a different project, and it will answer your requests and render someone else's page. Confirm identity from the response — a title, a known route, a health payload — before testing against it.
...
- **A delegate that failed or stalled has usually already changed the tree.** A timeout, a watchdog kill or an error is not a rollback: the edits it made before dying are applied, and the verification it was told to run is the part that did not happen. So the failure mode is not missing work, it is *unverified work that looks finished* — the diff is there, the typecheck may even pass, and nothing marks which files were left half-finished. Never report a branch complete on the strength of "the other agents succeeded". For every delegate that did not return a clean report, run its verification yourself, and say in your own summary which files were covered that way rather than by the agent. Where several agents share a branch, re-run the whole gate afterwards: a partial edit only conflicts with a sibling's edit once both are in the tree.
- **A delegate must never adjust an assertion to match its own output.** Given a failing check, the cheapest move available to it is editing the expectation, and it will often do so and then report success in good faith — the test passes, the diff looks deliberate, and the regression is now pinned as correct. Say explicitly in the prompt that a check which fails after its change is a finding to report, not a value to update, and that touching test expectations is out of scope unless you asked for it. When a delegate's report does mention changing an assertion, treat it as unreviewed: read the old and new expectation yourself and establish which one describes the behaviour you want, because a plausible justification is exactly what this mistake comes wrapped in.
- **Separate a delegate's measurements from its interpretations** — a subagent report mixes counts and log lines it actually read with the causal story it wrapped around them. The measurements are usually sound; the story often is not, because the delegate saw one slice and had every incentive to explain it. Verify the mechanism yourself before repeating a causal claim in your own voice, and above all before it lands somewhere durable like a PR description, a ticket, a tracker's closure note, or a message to a team. "X is 404ing, so Y is broken" can be an accurate count welded to an invented cause — and the count is what makes the cause sound checked.
...
**Per call, effort is a narrower surface than model.** `opts.effort` (`low | medium | high | xhigh | max`) exists on Workflow nodes; omit it to inherit the session effort. The **Agent tool has no `effort` parameter** — writing one into an Agent call does nothing, so a subagent that should be cheap gets a cheaper `model` instead. A subagent or skill *definition* can pin `effort:` in its frontmatter, but that fixes it for every call, which is why the definitions in `agents/` leave it out, as they leave out `model:`. Session effort is the user's to set, same as session model.

## Saved workflows

A good run can be saved for reuse — version-controlled and viewable live with `/workflows`. Built-in workflows and scripts in `.claude/workflows/` or `~/.claude/workflows/` resolve by name (`/<name>`); `scriptPath` launches any script file by path. Global orchestration guidance lives here; a workflow scoped to a single repo lives in that repo's `.claude/workflows/`. Running any saved workflow still requires opt-in. Worked example: `.claude/workflows/config-consistency-audit.js` (fan out over subsystems → verify each finding → synthesize a ranked report).

Don't document a launch or discovery mechanism as working until you've run it once. Name resolution, flags, and auto-discovery vary by harness version, and a failed launch describes that version only. State the verified behavior; don't promise the plausible one.
