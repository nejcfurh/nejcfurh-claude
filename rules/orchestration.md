...
- **A delegate that failed or stalled has usually already changed the tree.** A timeout, a watchdog kill or an error is not a rollback: the edits it made before dying are applied, and the verification it was told to run is the part that did not happen. So the failure mode is not missing work, it is *unverified work that looks finished* — the diff is there, the typecheck may even pass, and nothing in the tree marks which files were left mid-review. Never report a branch complete on the strength of "the other agents succeeded". For every delegate that did not return a clean report, run its verification yourself, and say in your own summary which files were covered that way rather than by the agent. Where several agents share a branch, also re-run the whole gate afterwards: a partial edit only conflicts with a sibling's edit once both are in the tree.
