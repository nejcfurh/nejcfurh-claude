...
- **Thinking defaults differ by model, so never assume one.** On Opus 5 thinking is on when the field is omitted; on Opus 4.8/4.7 omitting it meant off. Read the target model's default before costing anything.
- **Lower `effort`, don't disable thinking.** `thinking: {type: "disabled"}` on Opus 5 has two silent failure modes: the model sometimes writes a tool call into its visible text instead of a `tool_use` block (the turn succeeds, the call never runs, nothing errors — worst in an agentic loop, and most common on tool-heavy work), and `<thinking>` tags can leak into the response. `low`/`medium` effort captures most of the saving without either. Opus 5 also rejects disabled thinking above `high` effort. If a route must stay thinking-off, delete any "don't think/don't reason" instruction (it worsens tag leakage), don't name thinking tags, and give the model explicit permission to say a sentence before a tool call.
...
- **Writes cost more than reads, and every multiplier here is per-model.** The shape is stable: a cache write costs more than an uncached read, a cache read costs a fraction of one, and the 1h TTL costs more to write than the 5m — so break-even is a number of reads, and the longer TTL needs more of them. The rates are not stable. The read multiplier alone varies by roughly an order of magnitude across current models, which moves every break-even with it. Read the rates for the model actually in play — `/claude-api` covers it, the pricing page is the source — and derive the break-even from those. A multiplier remembered from another model makes the whole estimate fiction while looking arithmetically sound.
- **Caching and wide batching fight each other.** A cache entry is only readable after the first response begins streaming, so N parallel requests sharing a prefix all pay the write. On a bulk backfill — the exact case where the Batch API's 50% is most tempting — the cached-prefix saving does not materialise unless you warm the prefix serially first or accept the higher 1h-TTL write premium. Model the two together or the estimate is fiction.
