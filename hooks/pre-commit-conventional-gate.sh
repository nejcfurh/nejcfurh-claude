...
if printf '%s\n' "$cmd" | grep -Eq -- '(^|[[:space:]])-[Cc]([[:space:]]|$)'; then
  exit 0
fi
...
if printf '%s\n' "$subject" \
  | grep -Eq '^(feat|fix|refactor|perf|docs|style|test|build|ci|chore|revert)(\([a-zA-Z0-9./_-]+\))?!?: .+'; then
  exit 0
fi

{
  ...
  echo "  Types:    feat fix refactor perf docs style test build ci chore revert"
