...
- **A recurring activity gets one memory file, not one per run.** A weekly triage, a repeated review, a periodic sweep — each run wants to record what it found, and the cheap move is a dated file per run. That adds an index line every run, and the index is charged against every future session's context whether or not the memory is ever used. Keep one file per activity with dated sections inside it, and one index line naming the classes that recur. The file may grow freely; only the index line costs.
